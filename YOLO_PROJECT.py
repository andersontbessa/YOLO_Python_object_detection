...
while (cap.isOpened()):
    ...
    for (classId, score, box) in zip(classes, scores, boxes):
        ...
        cv2.putText(frame, label, (box[0], box[1]-10), 0, 0.5, (0,255,0), 2)

    color = (0, 255, 0)
    thickness = 2
    cv2.imshow('frame', frame)
    cv2.line(frame, (division_y, x), (division_y, 1), color, thickness)
    cv2.putText(frame, 'remote_pass = {}'.format(remote_pass), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    cv2.putText(frame, 'cell_phone_pass = {}'.format(cell_phone_pass), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                (0, 255, 255), 2)
    cv2.putText(frame, 'cup_pass = {}'.format(cup_pass), (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    cv2.imshow('frame', frame)
